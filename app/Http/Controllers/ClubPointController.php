...
class ClubPointController extends Controller
{
    ...
    public function point_for_review_store(Request $request)
        {
            $club_point_for_review = BusinessSetting::where('type', $request->type)->first();
            if ($club_point_for_review != null) {
                $club_point_for_review->value = $request->value;
            }
            else {
                $club_point_for_review = new BusinessSetting;
                $club_point_for_review->type = $request->type;
                $club_point_for_review->value = $request->value;
            }
            $club_point_for_review->save();
            
            Artisan::call('cache:clear');
            
            flash(translate('Point for review has been updated successfully'))->success();
            return redirect()->route('club_points.configs');
        }
